...
- [ ] **P2-4 一時間wait loopとcursor回復を実装する。**
  ...
  - [x] 公開`observer watch`／`watch start|status|stop`を両provider共通の非H lifecycle handlerへ接続する
    （[ADR 0095](adr/0095-public-watch-lifecycle-cli-contract.md)）。
    - [x] 利用者入力をabsolute project pathだけに保ち、現在親のprovider／runtime／authorizationとhost actionを
      argv外のexact contextへ分離する。
    - [x] startのreserve→spawn receipt耐久化→ready、sanitized status、terminal receipt後だけのstop完了を、
      Claude／Codex共通result schemaとfake host actionで固定する。
    - 実装`a4195a3`、focused 13/13、関連37/37、static greenを
      [ADR 0096](adr/0096-public-watch-lifecycle-cli-acceptance.md)で受け入れた。
          ...
          - [ ] model request送信結果不明をhost lifecycleと別journalで回収する
            ...
            - [ ] Claude／Codexのexact operation result readをprovider固有journalへ実装し、handle欠損を
              ...
              - [ ] 旧Claude background job経路のblockedを維持し、Aiterm公開対話transportをClaude callerへ接続する。
                ...
                - [x] Stop未発火とpayload／result不正を区別するraw-free diagnostic receiptを
                  非Hで実装し、focused 9/9、related 29/29で受け入れた
                  （[ADR 0113](adr/0113-claude-characterization-diagnostic-receipt-acceptance.md)）。
                - [x] diagnostic receipt受入後、別H承認で一つのjobだけを再characterizeした。
...
- [x] **P3-1 中央Mailboxの保存・publish契約を実装する。**
  ...
  - [x] `dedupe_key`が既存secret pattern検査から漏れている欠陥を
    [ADR 0085](adr/0085-mailbox-dedupe-key-secret-correction.md)で独立補正した。
    修正前focused 6/7で再現し、commit `ae7e336`、focused 7/7、関連30/30、static gateを
    [ADR 0086](adr/0086-mailbox-dedupe-key-secret-correction-acceptance.md)で受け入れた。
...
- [x] **P4-1 Observer起動契約と出力schemaを実装する。**
  ...
  - [x] P4-2の信頼済みcycle contextとP4-3のsemantic gateを通した後、Mailbox publishより前に
    Supervisorが本parserを必須実行し、provider／target／watch／cycleをAI自己申告なしで束縛した
...
- [x] **P4-3 過剰指摘抑制を実装する。**
  ...
  - [x] **P4-3b cycle application integration:** acceptedだけをMailboxへexact publishし、suppressed専用result、
    ...
    （[ADR 0089](adr/0089-advisory-semantic-cycle-integration-acceptance.md)）。
  - [x] **P4-3c semantic behavioral eval:** materiality、actionability、semantic timing、親が対処中かを
    ...
    [ADR 0091](adr/0091-semantic-behavioral-eval-acceptance.md)）。
...
- [ ] **P5-1 E2Eとfault injectionを完了する。**
  ...
  - [x] **P5-1b3 Claude public surface characterization／代替transport確定:** 公開非対話reply ACK、exact result read、
    ...
    - [x] **P5-1b3c 非H diagnostic receipt:** Stop未発火とstdin／payload／result不正をraw-freeに
      ...
      [ADR 0113](adr/0113-claude-characterization-diagnostic-receipt-acceptance.md)）。
    - [x] **P5-1b3d live H再characterization:** 別H承認で一つのbackground job／Haiku requestだけを
  ...
  - [x] **P5-1b5 dual-host live H:** Aiterm実Claude初回／follow-up、Stop、exact
    ...
    - [x] **P5-1b5a preflight／runbook production route補正:** P5-1b4完了後も残った旧
      ...
      Stop hook候補を
      read-only preflightで確認する。live手順は
      `observer parent claude run ... --aiterm-command ...`と
    ...
    - [x] **P5-1b5b 通常系dual-host live H:** 承認済み通常campaignだけを各host一回実行し、preflight
      ...
      - [x] **P5-1b5b-r2 Throughline capture実行物束縛:** state root修理後のClaude attemptで
        ...
        - [x] **P5-1b5b-r3 Aiterm runtime隔離:** controller processのPATHを補正してもglobal tmux
        ...
        - [x] **P5-1b5b-r9 SIGINT時のAiterm子process signal隔離:** r8候補の正常停止で、foreground
        ...
        - [x] **P5-1b5b-r10 cycleごとのexact output contract:** r9修理後のfresh Claude candidateで
        ...
        - [x] **P5-1b5b-r16 post-spawn／pre-ready recovery訂正:** 独立監査候補を親が実コードで
          ...
          （[ADR 0142](adr/0142-post-spawn-pre-ready-recovery-correction.md)）。実装はcommit `4bde91c`へ
          独立固定し、focused 19/19、parent-launch／watch-store／両Supervisor related 33/33、
          `npm run check`がgreen。受入証拠は
          [ADR 0143](adr/0143-post-spawn-pre-ready-recovery-acceptance.md)。
...
- [x] **P5-4 Throughline上位互換version gateを補正する。**
  ...
  - 完了条件: Throughline `0.8.7`とそれより新しい安定版を受理し、旧版・不正SemVer・prereleaseを拒否する。
    version受理後も`observer-read`／`observer-wait`のstrict wire検証を維持し、互換性をversion文字列だけで
    成功扱いしない。
  - [x] version range契約とverification schemaを[ADR 0145](adr/0145-throughline-forward-version-compatibility.md)へ固定する。
  - [x] runtime、preflight、diagnostics、focused testを補正する。
  - [x] Codex CLIも最低対応版以上＋actual version束縛へ揃え、
