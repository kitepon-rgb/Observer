...
export function buildObserverAiPrompt(childStart) {
  ...
  return [
    ...
    "既定はno_advisoryです。advisoryは次の全条件を満たす場合だけ選んでください。",
    "claimがbounded evidenceから具体的に確認でき、放置すると目的・scope・安全・受入条件・検証へ実質的な影響があります。",
    "親が検討できる一つの具体的行動があり、未解決で今伝える価値があり、親が認識して具体的な対処を開始していません。",
    "単なる好み、別解、一般論、励まし、軽微な最適化、証拠不足、解決済みならno_advisoryを選んでください。",
