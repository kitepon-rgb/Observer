...
async function advanceStop(args) {
  ...
  if (provider === "claude") {
    if (isAitermClaude(launchRequest)) {
      ...
      stopped = closed.terminal_receipt;
  ...
  await core.confirmGenerationHostTerminal({
    stateRoot,
    targetId,
    watchId,
    terminalReceipt: stopped,
    stopCommandReceipt,
  });
...
async function recoverSpawn(args) {
  ...
  if (launchRequest.provider === "claude") {
    if (isAitermClaude(launchRequest)) {
      const recovered = await providers.aitermClaude.recoverAitermClaudeSpawn({
        stateRoot,
        request: launchRequest,
        transport: session,
      }, providerDependencies.aitermClaude);
      if (recovered.receipt === null) return result(context, "unknown", recovered.reason ?? "provider_spawn_unavailable");
      spawn = recovered.receipt;
      ready = (await providers.aitermClaude.activateAitermClaudeGeneration({
        stateRoot,
        request: launchRequest,
        receipt: spawn,
...
function isAitermClaude(request) { return request.provider === "claude" && request.required_handle_kind === "claude.session"; }
